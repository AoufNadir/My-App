...
import { InvestorPerformanceChart } from '../src/components/dashboard/InvestorPerformanceChart';
...
export const InvestorDashboardPage: React.FC<InvestorDashboardPageProps> = ({ investor, transactions, isDark, globalNetProfit, managerFeePercentage, totalCapital }) => {
    ...
    const handleDownloadReport = () => {
        alert("La fonctionnalité de téléchargement sera disponible bientôt.");
    };

    const handleRequestWithdrawal = () => {
        // In a real app, this would open a modal or send a request to backend
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-slate-900 text-white' : 'bg-slate-50 text-gray-900'} p-4 md:p-8 space-y-6 pb-24`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border shadow-sm`}>
                    <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold mt-1">{investor.capitalInvested.toLocaleString('fr-FR')} <span className="text-sm font-normal text-gray-500">DZD</span></p>
                ...
                <Card className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border shadow-sm relative overflow-hidden`}>
                    <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-bl-full -mr-2 -mt-2"></div>
                    <CardContent className="p-4">
                        <p className={`text-sm font-medium ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Valeur Actuelle (Estimée)</p>
                        <p className="text-2xl font-bold mt-1 text-indigo-500">{stats.totalValue.toLocaleString('fr-FR')} <span className="text-sm font-normal text-gray-500">DZD</span></p>
                        <p className={`text-xs mt-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Capital + profits non retirés</p>
                    </CardContent>
                </Card>
                ...
                <Card className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border shadow-sm`}>
                    <CardContent className="p-4">
                        ...
                        <p className="text-2xl font-bold mt-1 text-emerald-500">+{stats.currentTotalProfit.toLocaleString('fr-FR')} <span className="text-sm font-normal text-gray-500">DZD</span></p>
                        <div className="mt-2 text-xs flex items-center gap-1 text-emerald-500">
                            <TrendingUpIcon className="w-3 h-3" />
                            <span className="font-medium">+{stats.profitPercentage.toFixed(2)}% de rendement</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border shadow-sm`}>
                    <CardContent className="p-4">
                        <p className={`text-sm font-medium ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Durée d'investissement</p>
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {transactions.length === 0 ? (
                                    <tr>
                                        <td colSpan={3} className="px-4 py-8 text-center opacity-50">Aucune transaction trouvée.</td>
                                    </tr>
                                ) : (
                                    [...transactions].sort((a, b) => b.timestamp - a.timestamp).map((tx) => (
                                        <tr key={tx.id} className={`${isDark ? 'hover:bg-slate-700/30' : 'hover:bg-slate-50'} transition-colors`}>
                                            ...
                                            <td className="px-4 py-3">
                                                ...
                                                ${tx.type === 'profit_distribution' ? 'bg-emerald-500/10 text-emerald-500' :
                                                        tx.type === 'deposit_capital' ? 'bg-blue-500/10 text-blue-500' :
                                                            tx.type === 'withdraw_profit' ? 'bg-amber-500/10 text-amber-500' :
                                                                'bg-gray-500/10 text-gray-500'}`}>
                                                    {tx.type === 'profit_distribution' ? 'Distribution Profit' :
                                                        tx.type === 'deposit_capital' ? 'Dépôt Capital' :
                                                            tx.type === 'withdraw_profit' ? 'Retrait Profit' : 'Retrait Capital'}
                                                </span>
                                            </td>
                                            <td className={`px-4 py-3 text-right font-bold ${['profit_distribution', 'deposit_capital'].includes(tx.type) ? 'text-emerald-500' : 'text-red-500'
                                                }`}>
                                                {['profit_distribution', 'deposit_capital'].includes(tx.type) ? '+' : '-'}{tx.amount.toLocaleString('fr-FR')} DZD
                                            </td>
                                        </tr>
                                    ))
                                )}
            ...
            <div className="text-center text-xs opacity-50 pb-8">
                <p>&copy; {new Date().getFullYear()} Pro Digital Investment. Tous droits réservés.</p>
                <p className="mt-1">Les performances passées ne préjugent pas des performances futures.</p>
            </div>
