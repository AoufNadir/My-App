      ...
      <Card className={cardBase}>
        <CardHeader className="p-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
                 <Button onClick={() => openSettlementModal('reçu')} className="flex-1 text-sm bg-green-600/80 hover:bg-green-700/80 text-white font-semibold py-2 px-3 rounded-lg">Règlement</Button>
                 <Button onClick={() => openSettlementModal('effectué')} className="flex-1 text-sm bg-red-600/80 hover:bg-red-700/80 text-white font-semibold py-2 px-3 rounded-lg">Paiement</Button>
                 <Button onClick={() => setIsTransferModalOpen(true)} className="p-2 rounded-lg bg-sky-600/80 hover:bg-sky-700/80 text-white"><ArrowRightLeftIcon className="w-5 h-5"/></Button>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-4 pt-0">
            <div className="flex gap-2 mb-4">
                ...
                <Button onClick={() => openClientModal(null)} className="p-2.5 rounded-lg bg-amber-500/80 hover:bg-amber-600/80 text-white"><PlusIcon className="w-5 h-5" /></Button>
            </div>
            ...
            {filteredClientsDzd.length > 0 ? (
                <div className="divide-y" style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
                    {filteredClientsDzd.map(client => {
                        ...
                        return (
                            <div key={client.id} onTouchStart={() => handleTouchStart(client)} onTouchEnd={handleTouchEnd} onContextMenu={(e) => { e.preventDefault(); handleTouchStart(client); }}
                                className="flex items-center gap-3 p-4 cursor-pointer" onClick={() => setSelectedClientId(client.id)}>
