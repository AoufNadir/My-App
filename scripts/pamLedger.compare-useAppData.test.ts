...
import { computePamLedger, type PamCurrency } from '../src/utils/pamLedger.js';
import type { PortfolioStats, Tx } from '../src/types';

type ComparisonStatus =
  | 'match'
  | 'rounding_only'
  | 'legacy_fallback_difference'
  | 'manual_adjustment_related'
  | 'eur_conversion_related'
  | 'chronological_order_difference'
  | 'actual_difference';
...
type ComparisonRow = {
  source: string;
  currency: PamCurrency | 'ALL';
  metric: ComparableMetric;
  currentUseAppData: number | null;
  pamLedger: number | null;
  difference: number;
...
function normalizeTx(raw: Record<string, unknown>, index: number): Tx {
  const tx = { ...raw };
  ...
  return {
    ...
    type: String(tx.type || '') as Tx['type'],
    ...
    clientPaymentStatus: tx.clientPaymentStatus as Tx['clientPaymentStatus'],
    paymentMethod: tx.paymentMethod as Tx['paymentMethod'],
  };
}

function extractTransactions(input: any): Tx[] {
  ...
  if (Array.isArray(input)) rows = input;
    ...
    rows = input.documents.map((doc: any) => ({
...
function computeUseAppDataPortfolioStats(transactions: Tx[]): PortfolioStats {
...
function classifyDifference(
...
): { status: ComparisonStatus; reason: string } {
  ...
  if (ledgerWarnings.includes('uncosted_quantity_sold')) {
    return {
      status: 'manual_adjustment_related',
      reason: 'Ledger contains sells with quantity that has no historical cost basis; usually caused by quantity-only manual stock adjustments or stock gaps.',
    };
  }
  if (ledgerWarnings.includes('oversell')) {
    return {
      status: 'actual_difference',
      reason: 'Ledger contains sell rows where quantity exceeds available historical stock.',
    };
  }
...
function compareFile(filePath: string) {
  ...
  for (const currency of ['USDT', 'EUR'] as PamCurrency[]) {
...
function main(): void {
  ...
  const materialDifferences = comparisons.flatMap((comparison) => (
    comparison.rows.filter((row) => (
      row.metric !== 'storedProfitTotal'
