...
// src/utils/money.ts
var toCents = (n) => Math.round((n || 0) * 100);
var fromCents = (c) => c / 100;
var addM = (a, b) => fromCents(toCents(a) + toCents(b));
var subM = (a, b) => fromCents(toCents(a) - toCents(b));
var sumM = (values) => fromCents(values.reduce((acc, v) => acc + toCents(v), 0));
var roundM = (n) => fromCents(toCents(n));
var distributeProportionally = (total, weights) => {
...
function buildInvestorsBase(investors, investorTransactions) {
  const txByInvestor = /* @__PURE__ */ new Map();
  for (const tx2 of investorTransactions) {
    const list = txByInvestor.get(tx2.investorId) || [];
    list.push(tx2);
    txByInvestor.set(tx2.investorId, list);
  }
  return investors.map((inv) => {
    const myTxs = txByInvestor.get(inv.id) || [];
    const movementTxs = myTxs.filter(
      (tx2) => tx2.type === "deposit_capital" || tx2.type === "reinvest_profit" || tx2.type === "withdraw_capital"
    );
    const currentCapitalFromMovements = movementTxs.reduce((sum, tx2) => {
      if (tx2.type === "withdraw_capital") return subM(sum, tx2.amount);
      return addM(sum, tx2.amount);
    }, 0);
    const withdrawnProfit = myTxs.filter((tx2) => tx2.type === "withdraw_profit").reduce((sum, tx2) => addM(sum, tx2.amount), 0);
    const reinvestedProfit = myTxs.filter((tx2) => tx2.type === "reinvest_profit").reduce((sum, tx2) => addM(sum, tx2.amount), 0);
    return {
      ...inv,
...
function capitalAtTs(inv, ts) {
  const movementsUntilTs = inv.txs.filter(
    (tx2) => toMs(tx2.timestamp) <= ts && (tx2.type === "deposit_capital" || tx2.type === "reinvest_profit" || tx2.type === "withdraw_capital")
  );
  if (movementsUntilTs.length === 0) {
    return inv.hasCapitalMovements ? 0 : inv.initialCapital;
  }
  return movementsUntilTs.reduce((sum, tx2) => {
    if (tx2.type === "withdraw_capital") return subM(sum, tx2.amount);
    return addM(sum, tx2.amount);
  }, 0);
}
function chronologicalDerivedSells(pamLedger) {
  return [...pamLedger.sellProfitRows].filter((row) => Number.isFinite(Number(row.derivedProfit)) && Number(row.derivedProfit || 0) !== 0).sort((a, b) => toMs(a.timestamp) - toMs(b.timestamp));
}
function deriveInvestorEconomics(input) {
  ...
  const distributedProfitByInvestor = /* @__PURE__ */ new Map();
  const warningsByInvestor = /* @__PURE__ */ new Map();
  const warnings = [];
  ...
  for (const sellRow of chronologicalDerivedSells(pamLedger)) {
    ...
    const eligible = investorsBase.filter((inv) => inv.entryTs <= sellTs).map((inv) => ({ id: inv.id, cap: Math.max(0, capitalAtTs(inv, sellTs)) })).filter((item) => item.cap > 0);
  ...
  const derivedInvestors = investorsBase.map((inv) => {
    const currentShare = inv.isActive && totalCurrentCapital > 0 ? Math.max(0, inv.capitalInvested) / totalCurrentCapital : 0;
...
function investor(input) {
  const capital = input.initialCapital ?? input.capitalInvested ?? 0;
  return {
    name: input.id,
    entryDate: (/* @__PURE__ */ new Date(0)).toISOString(),
...
test("investor distribution uses derived PAM profit instead of stored tx.profit", () => {
  const result = deriveInvestorEconomics({
    investors: [investor({ id: "inv-a", initialCapital: 1e5 })],
...
test("capital deposited before a sell participates, capital deposited after the sell does not", () => {
  ...
  const result = deriveInvestorEconomics({
    ...
    investorTransactions: [
      investorTx({ id: "before-deposit", investorId: "before", type: "deposit_capital", amount: 1e3, timestamp: sellTs - 1e3 }),
      investorTx({ id: "after-deposit", investorId: "after", type: "deposit_capital", amount: 1e3, timestamp: sellTs + 1e3 })
    ],
    transactions: [
      tx({ id: "buy", timestamp: sellTs - 2e3, type: "buy", quantity: 100, price: 100, total: 1e4 }),
      tx({ id: "sell", timestamp: sellTs, type: "sell", quantity: 100, sell: 120, profit: 2e3 })
    ],
  ...
  assertMoney(result.totals.investorShare + result.totals.managerShare, 2e3);
  assertMoney(result.totals.reconciliationDifference, 0);
});
test("manager fee plus investor shares reconciles exactly to derived profit", () => {
  const result = deriveInvestorEconomics({
    ...
    investorTransactions: [],
    transactions: [
      tx({ id: "buy", timestamp: 1e3, type: "buy", quantity: 100, price: 100, total: 1e4 }),
      tx({ id: "sell", timestamp: 2e3, type: "sell", quantity: 100, sell: 101, profit: 100 })
    ],
    managerFeePercentage: "0"
  });
...
test("negative derived profit keeps current behavior and emits a warning", () => {
  const result = deriveInvestorEconomics({
    investors: [investor({ id: "loss-investor", initialCapital: 1e3 })],
    investorTransactions: [],
    transactions: [
      tx({ id: "buy", timestamp: 1e3, type: "buy", quantity: 100, price: 100, total: 1e4 }),
      tx({ id: "loss-sell", timestamp: 2e3, type: "sell", quantity: 100, sell: 90, profit: -1e3 })
    ],
    managerFeePercentage: "20"
  });
...
test("negative availableProfit is warning-only when withdrawals exceed derived totalProfit", () => {
  const result = deriveInvestorEconomics({
    investors: [investor({ id: "over-withdraw", initialCapital: 1e5 })],
    investorTransactions: [
      investorTx({
...
test("uncostedQuantitySold creates investor accounting warning without blocking distribution", () => {
  const result = deriveInvestorEconomics({
    investors: [investor({ id: "uncosted-investor", initialCapital: 1e3 })],
    investorTransactions: [],
    transactions: [
      tx({ id: "buy", timestamp: 1e3, type: "buy", quantity: 100, price: 100, total: 1e4 }),
      tx({ id: "qty-only", timestamp: 2e3, type: "Ajout Manuel", quantity: 10, price: 0 }),
      tx({ id: "sell-uncosted", timestamp: 3e3, type: "sell", quantity: 105, sell: 110, profit: 1050 })
    ],
    managerFeePercentage: "0"
  });
  const row = result.derivedInvestors[0];
