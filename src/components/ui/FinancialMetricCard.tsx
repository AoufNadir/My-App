...
const toneClasses = {
    ...
    stock: {
        icon: 'text-financial-asset',
        bg: 'bg-financial-asset-bg',
        ring: 'ring-primary/15'
    },
    cash: {
        icon: 'text-financial-profit',
        bg: 'bg-financial-profit-bg',
        ring: 'ring-success/15'
    },
    debt: {
        icon: 'text-financial-loss',
        bg: 'bg-financial-loss-bg',
        ring: 'ring-danger/15'
    },
    profit: {
        icon: 'text-financial-profit',
    ...
    return (<div role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined} onClick={onClick} onKeyDown={(event) => {
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-[11px] font-bold uppercase text-neutral-500">
            {label}
          </p>
          <div className="mt-1">
            {valueDisplay ?? <MoneyText value={value} currency={currency} semantic={semantic} size="lg"/>}
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          {onEdit && (<button type="button" onClick={(event) => {
                event.stopPropagation();
                onEdit();
            }} className="flex h-touch w-touch items-center justify-center rounded-lg bg-neutral-100 text-neutral-600 transition-colors hover:bg-neutral-200" aria-label={`Modifier ${label}`}>
              <PencilIcon className="h-4 w-4"/>
            </button>)}
          {icon && (<div className={`flex h-9 w-9 items-center justify-center rounded-lg ${toneClass.bg} ${toneClass.icon}`}>
              {icon}
            </div>)}
        </div>
      </div>
