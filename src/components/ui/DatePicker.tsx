...
export const DatePicker = React.forwardRef<HTMLInputElement, DatePickerProps>(function DatePicker({ value, onChange, className, placeholder, disabled, min, max, id, ariaLabel }, ref) {
    ...
    return (<div className={`relative flex items-center ${className ?? ''}`}>
      <input ref={ref} id={id} type="text" inputMode="numeric" value={display} onChange={handleTextChange} onBlur={handleBlur} placeholder={placeholder ?? 'DD/MM/YYYY'} disabled={disabled} aria-label={ariaLabel} className={`flex min-h-input w-full rounded-button border px-3 py-2 pe-12 text-sm outline-none transition disabled:cursor-not-allowed disabled:opacity-50 ${baseClass} ${invalidClass}`}/>
      <button type="button" onClick={openNativePicker} disabled={disabled} aria-label="Open date picker" tabIndex={-1} className="absolute end-1 inline-flex h-icon-button w-icon-button items-center justify-center rounded-button text-neutral-500 transition hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-40">
