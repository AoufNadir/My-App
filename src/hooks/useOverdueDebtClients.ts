...
export function useOverdueDebtClients({
  clients,
  clientTransactions,
  clientBalances,
  getClientFullName,
  minDays = 7
}: UseOverdueDebtClientsArgs) {
  return useMemo<OverdueDebtClient[]>(() => {
    ...
    for (const client of clients) {
      ...
      if (currentBalance >= -EPSILON) continue;

      const clientTxs = (txByClient.get(client.id) || []).slice().sort((a, b) => a.timestamp - b.timestamp);
      ...
      if (overdueLots.length === 0) continue;

      const currentDebtAbs = Math.abs(Math.min(currentBalance, 0));
      // Display the real current debt to keep parity with client detail screen.
