...
type PortfolioSnapshot = {
  ...
  eur: { available: number; avgBuy: number };
};
...
export function buildMonthlyPdfReport(input: MonthlyReportInput): ReportPayload {
  ...
  for (const tx of periodTxs) {
    ...
    if (tx.currency === 'USDT' && tx.type === 'sell') {
      ...
      realizedProfit += Number(tx.profit || 0);
      sellCount += 1;
    }
  ...
  for (const tx of periodTxs) {
    ...
    if (tx.type === 'buy') row.buyVolumeUsdt += tx.quantity;
    if (tx.type === 'sell') {
  ...
  const bodyHtml = `
    ...
    <section class="section">
      ...
      <div style="margin-top: 10px;" class="pill">
        Profit net cumule: <strong class="${input.portfolioStats.usdt.totalProfit >= 0 ? 'good' : 'bad'}">${input.portfolioStats.usdt.totalProfit >= 0 ? '+' : ''}${formatNumber(input.portfolioStats.usdt.totalProfit)} DZD</strong>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div class="cards">
        ...
        <div class="card">
          ...
          <div class="value good">+${formatNumber(input.investor.totalProfit || 0)} DZD</div>
        </div>
        <div class="card">
          <div class="label">Profit disponible</div>
          <div class="value ${((input.investor.availableProfit || 0) >= 0) ? 'good' : 'bad'}">${(input.investor.availableProfit || 0) >= 0 ? '+' : ''}${formatNumber(input.investor.availableProfit || 0)} DZD</div>
        </div>
      </div>
      <div style="margin-top: 10px;">
